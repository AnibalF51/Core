...
<body>
    ...
    <div class="card-body">
        ...
        <div class="row">
            <p class="col-sm-5">Reporte correpondiate al dia: {{ $fecha }}</p>
    ...
    <div class=" card card-primary card-outline">
        ...
        <table id="tableexample1" class="table table-sm ">
            ...
            <tbody id="prin">
                ...
                @foreach ($bol as $bo)
                    @if ($bo->estado == 'Aprobado' and $bo->created_at->format('Y-m-d') == $fecha and $bo->CodEmpresa == auth()->user()->codempresa)
                        <tr>
                            <td>{{ $bo->id }}</td>
    ...
    <div class=" card card-primary card-outline">
        ...
        <table id="tableexample1" class="table table-sm ">
            ...
            <tbody id="prin">
                ...
                @foreach ($bol as $bo)
                    @if ($bo->estado == 'Pendiente' and $bo->created_at->format('Y-m-d') == $fecha and $bo->CodEmpresa == auth()->user()->codempresa)
                        <tr>
                            <td>{{ $bo->id }}</td>
                            <td>{{ $bo->nBoleta }} / Q {{ $bo->vBoleta }}</td>
                            <td>{{ $bo->nRecibo }} / Q {{ $bo->mRecibo }}</td>
    ...
    <div class=" card card-primary card-outline">
        ...
        <table id="tableexample1" class="table table-sm ">
            ...
            <tbody id="prin">
                ...
                @foreach ($bol as $bo)
                    @if ($bo->estado == 'Anulado' and $bo->created_at->format('Y-m-d') == $fecha and $bo->CodEmpresa == auth()->user()->codempresa)
                        <tr>
                            <td>{{ $bo->id }}</td>
                            <td>{{ $bo->nBoleta }} / Q {{ $bo->vBoleta }}</td>
                            <td>{{ $bo->nRecibo }} / Q {{ $bo->mRecibo }}</td>
